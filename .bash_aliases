...
alias g-set-auto-upstream="git config --global push.autoSetupRemote true" # Alternative to git push --set-upstream origin main
...
alias d-compose-install="curl -SL https://github.com/docker/compose/releases/download/v2.25.0/docker-compose-linux-x86_64 \
  -o ~/.docker/cli-plugins/docker-compose" # Install docker compose next make executable with chmod +x ~/.docker/cli-plugins/docker-compose
...
alias snap-intellij-install="sudo snap install intellij-idea --classic" # Install using snap. snap find "intellij"
...
alias gradle-init="gradle init --type java-application --dsl groovy --test-framework junit-jupiter --package co.com.manuel.project.start --project-name project --no-split-project --java-version 21" # Start a gradle java project. Other example "gradle init" "gradle test" to run the test. https://docs.gradle.org/current/userguide/build_init_plugin.html#build_init_plugin
...
alias gradle-build="gradlew build -t" # To Build the gradle project. Include test command and re-buid to hot reload.
...
alias spring-init-kotlin="spring init -d=web --language=kotlin --build=gradle --type=gradle-project-kotlin --java-version=21 --artifact-id=my-kotlin-app --group-id=com.co.manuel my-kotlin-app" # Start a maven kotlin project.
...
alias spring-run-maven="SERVER_PORT=4000 mvn spring-boot:run" # Hot Reload, run in dev, add to pom.xml <dependency> <groupId>org.springframework.boot</groupId> <artifactId>spring-boot-devtools</artifactId> <scope>runtime</scope> </dependency>
...
alias fun-find-file-type="sudo find . -type d -maxdepth 1" # to find directories d (regulars files use f) with maxdepth 1 indicate the starting directory and immediate children
